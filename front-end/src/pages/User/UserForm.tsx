...
export default function UserForm() {
  ...
  const { postUser, getUserById, patchUser } = UsersApi();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err.response?.status === 400 && err.response?.data?.errors) {
        ...
        setError(
          mode === 'edit' ? 'Impossible de modifier le profil' : 'Impossible de créer le compte',
        );
      }
    }
  }
        ...
        <div className="flex flex-col gap-6 rounded-xl border bg-white p-6">
          ...
          <div className="form-div">
            ...
            >
              <option value="" className="hidden">
                Sélectionnez si vous êtes un professionnel ou un particulier
              </option>
              <option value="particulier" className="bg-white text-black ">
                Particulier
              </option>
              <option value="professionnel" className="bg-white text-black ">
                Professionnel
              </option>
            </select>
          </div>
          ...
          <div className="flex gap-4 sm:flex-row mt-4">
            <button
              ...
              className=" flex-1 items-center h-10 rounded-md bg-white border border-gray-300 text-primary text-sm font-medium hover:bg-gray-300 transition cursor-pointer"
            >
              Annuler
            </button>
            <button
              type="submit"
              className=" flex-1 items-center h-10 rounded-md bg-accent text-white text-sm font-medium hover:bg-[#0087BB] transition cursor-pointer"
            >
              {mode === 'edit' ? 'Sauvegarder' : 'Créer le compte'}
            </button>
          </div>
        </div>
      </form>
