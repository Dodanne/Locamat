                        ...
                        <div key={r.rental_id} className="bg-white rounded-xl border p-4 space-y-4">
                         ...
                         <div className="flex gap-4 items-start">
                           <Link to={`/equipment/${r.equipment?.equipment_id}`}>
                             <img src={r.equipment?.photo} alt={r.equipment?.title} className="w-20 h-20 object-cover rounded-lg shrink-0"/>
                           </Link>
                           <div className="flex flex-col gap-1">
                             <h3 className="text-lg font-semibold text-gray-900">{r.equipment?.title}</h3>
                             <span className={`inline-flex items-center justify-center rounded-md border px-2 py-1 text-sm font-medium w-fit ${status[r.status].className}`}>
                         ...
                         <div className="flex items-center gap-3">
                            {r.equipment?.owner?.photo && r.equipment?.owner?.photo !== "NULL" ? (
                              <img src={r.equipment?.owner?.photo} className="w-10 h-10 object-cover rounded-full"/>
                            ) : (
                            ...
                            <div>
                              <p className="text-gray-700 text-sm">{r.equipment?.owner?.first_name} {r.equipment?.owner?.last_name}</p>
                              <div className="flex items-center text-gray-500 text-sm">
                                <IoLocationOutline/><span>{r.equipment?.owner?.city}</span>
                              </div>
                            </div>
                        </div>
                            ...
                            {r.status === "accepted" && (
                              <div className="flex flex-col gap-2 p-3 bg-gray-50 rounded-lg">
                                <p className="text-sm text-gray-700 text-center">
                                  {r.equipment?.owner?.last_name} a accepté votre demande. Pour confirmer, payez dès maintenant :
                                </p>
                                ...
                                <>
                                  {!showReview[r.rental_id] ? (
                                    hasReview[r.rental_id] ? (
                                      ...
                                      <button onClick={() => setShowReview(prev => ({...prev, [r.rental_id]: true}))} className="btn hover:bg-gray-300 w-full">
                                        Laisser un avis
                                      </button>
                                    )
                                  ) : (
                                     <Reviews rental={r} reviewSubmitted={() => {
