...
export default function ItemCard({
  ...
  const displayPhoto = form.file ? form.photo : equipment.photo;
  return (
    <div className="relative flex flex-col border rounded-lg overflow-hidden cursor-pointer hover:shadow-lg transition h-full bg-white">
          ...
          <div className="absolute bg-white/80 text-primary text-xs font-medium px-2 py-0.5 rounded-md border">
            {!isEditing ? (
              <>
                ...
                <button
                  onClick={() => onDelete?.(equipment.equipment_id)}
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-2 text-sm text-gray-600">
          ...
          <span className="text-sm text-black bg-primary/10 px-1 py-2 rounded-full text-center whitespace-nowrap">
        ...
        {editable && (
          <button
            className="text-primary btn border-primary bg-gray-100"
            onClick={() => navigate(`/equipment/${equipment.equipment_id}`)}
          >
            Accéder à la page du matériel
          </button>
        )}
      </div>
    </div>
