...
export default function ListeUtilisateurs() {
  ...
  const handleBan = async (userId: number, isBanned: boolean) => {
    if (!confirm(`Êtes-vous sûr de vouloir ${isBanned ? 'débannir' : 'bannir'} cet utilisateur ?`))
      return;
    try {
  ...
  const filteredUsers = users
    .filter((u) =>
      `${u.first_name} ${u.last_name}`.toLowerCase().includes(filterName.toLowerCase()),
    )
        ...
        <tbody>
          {filteredUsers.map((u) => (
            <tr key={u.user_id} className="hover:bg-gray-50">
              <td
                className="py-2 px-2 border cursor-pointer whitespace-nowrap "
                onClick={() => setFilterName(`${u.first_name} ${u.last_name}`)}
              >
                {u.last_name}
              </td>
              <td
