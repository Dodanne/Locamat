...
export const createEquipment = async (req, res) => {
  try {
    ...
    const photo = req.file
      ? req.file.path
      : "https://res.cloudinary.com/dxqyqpshr/image/upload/v1780487713/v1772622339default_ivnc80_ntnsus.png";
  ...
  try {
    ...
    if (data.owner_id !== req.user.id) {
      return res.status(403).json({
        message: "Vous n'avez pas la permission de supprimer ce matériel",
      });
    }
...
export const updateEquipment = async (req, res) => {
  try {
    ...
    if (data.owner_id !== req.user.id) {
      return res.status(403).json({
        message: "Vous n'avez pas la permission de modifier ce matériel",
      });
    }
    data.title = title;
