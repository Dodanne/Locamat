...
app.get("/equipment/:id", async (req, res) => {
  try {
    ...
    const [data] = await db.query(
      `SELECT e.*,u.id as id, u.user_type as user_user_type, u.city as user_city, u.rating_avg as user_rating_avg, u.rating_count as user_rating_count, u.first_name as user_first_name, u.last_name as user_last_name, u.photo as user_photo FROM equipment e JOIN users u ON e.owner_id =u.id WHERE e.id=${id}`
    );
    res.json(data[0]);
  } catch (err) {
    console.error(err);
